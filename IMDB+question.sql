...
SELECT SUM(title IS NULL) title_nulls,
SUM(year IS NULL) year_nulls,
SUM(date_published IS NULL) date_published_nulls,
SUM(duration IS NULL) duration_nulls,
SUM(country IS NULL) country_nulls,
SUM(worlwide_gross_income IS NULL) worldwide_gross_income_nulls,
SUM(languages IS NULL) languages_null,
SUM(production_company IS NULL) production_company_null
FROM movie;
...
SELECT SUM(name IS NULL) name_nulls,
SUM(height IS NULL) height_nulls,
SUM(date_of_birth IS NULL) date_of_birth_nulls,
SUM(known_for_movies IS NULL) known_for_movies_nulls
FROM names;
...
FROM
	...
	n.id = d.name_id
        ...
        m.id = d.movie_id
            ...
            m.id = r.movie_id
WHERE
	median_rating>=8 AND d.category='Actor'
GROUP BY
	name
ORDER BY
...
SELECT
    ...
    AVG(duration) OVER W2 AS moving_avg_duration
FROM
	movie m
    INNER JOIN
    genre g
...
,W2 AS(ORDER BY AVG(duration) ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING)
...
SELECT
    ...
    ROW_NUMBER() OVER(ORDER BY COUNT(m.id) DESC, AVG(avg_rating) DESC) AS actress_rank
FROM
	names n
    INNER JOIN
    role_mapping r
...
SELECT 
    ...
    title,
    date_published, 
	DATEDIFF(next_release_date,date_published) AS inter_movie_days
	FROM next_date
